...
import { AccountFilterNoRelations } from '../../components/account-filters/utils';
import { executeGraphql } from '../../generated/execute';
import { Account, AccountFilter, AccountsOrderBy } from '../../generated/graphql';
...
export const Accounts = () => {
  ...
  const variables = useMemo(
    () => ({
      ...(Object.keys(filters)?.length > 0 && { filter: filters as AccountFilter }),
  ...
  const { data, isFetching, isLoading, fetchNextPage, hasNextPage } = useInfiniteQuery({
    ...
    getNextPageParam: lastPage => lastPage?.allAccounts?.pageInfo?.hasNextPage ? lastPage?.allAccounts?.pageInfo?.endCursor : null,
  ...
  const handleFilterUpdates = useCallback((data: Partial<AccountFilterNoRelations>
  ) => {
    const startKeys = Object.keys(data);
    if (startKeys.length === 0) {
      setFilters({});
    };
    const inValidateData = Object.keys(data).flatMap(i => {
      const innerKeys = Object.keys(data[i as keyof Partial<AccountFilterNoRelations>]);
      if (innerKeys.length === 0) return false;
      return innerKeys.flatMap(j => {
        const val = data[i as keyof Partial<AccountFilterNoRelations>][j as any];
  ...
  return (
    <div>
      ...
      <AccountFilters initialValue={filters} handleChanges={handleFilterUpdates} />
      <AccountTable
      ...
      />
      <div>
        ({uniqBy(accounts || [], 'id').length} of {data?.pages?.[0]?.allAccounts?.totalCount || 0} rows fetched)
      </div>
    </div>
  );
};
