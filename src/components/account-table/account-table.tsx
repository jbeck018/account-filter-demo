...
import { TableInfiniteScroll, TableInfiniteScrollProps } from '../table-infinite-scroll/table-infinite-scroll';
import {
  StyledTable,
  TableBody,
  TableContainer,
...
export const AccountTable = ({ data, isLoading, isLoadingMore, fetchMore, hasNextPage }: AccountTableProps) => {
  ...
  const columns = useMemo(
    () => data?.length > 0 ?
      Object.keys(data?.[0] || [])
        .map(key => {
          if (key === 'usersByAccountId' || key === 'opportunitiesByAccountId') {
            return;
          ...
          return columnHelper.accessor(key as unknown as AccessorFn<Account>, {
            id: key,
