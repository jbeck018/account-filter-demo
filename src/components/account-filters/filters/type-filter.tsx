...
import { Option, PropertySelector, renderDropdownOption, SelectorBox } from '../../ui-components';
import { booleanValueOptions, enumFilterMap, enumOptions, RenderTypeMap, RenderTypes } from './filter-components';
...
export const TypeFilter = ({ name, value }: SharedFilterInputTypes) => {
  ...
  const handleValueChange = useCallback((item?: string | Option | Option[]) => {
    ...
    reset({
      ...currentValues,
      [name]: {
        [key]: isArray(item) ? item.map(i => i['id']) : isObject(item) ? item['id'] : item,
      },
    });
