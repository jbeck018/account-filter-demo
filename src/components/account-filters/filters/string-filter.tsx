...
import { MultiSearchSelectProps, Option, PropertySelector, renderDropdownOption, SelectorBox } from '../../ui-components';
import { AccountFilterNoRelations } from '../utils';
import { booleanValueOptions, RenderTypeMap, RenderTypes, stringFilterMap, textOptions } from './filter-components';
import { FlexRow, TextItem } from './fitler-utils';
import { SharedFilterInputTypes } from './types';
...
export const StringFilter = ({ name, value }: SharedFilterInputTypes) => {
  ...
  const handleValueChange = useCallback((item?: string | Option | Option[]) => {
    if (!item) return;
    const currentValues = getValues();
    reset({
      ...currentValues,
      [name]: {
        [key]: isArray(item) ? item.map(i => i[typeof i.id === 'boolean' ? 'id' : 'label']) : isObject(item) ? item[typeof item.id === 'boolean' ? 'id' : 'label'] : item,
      },
    });
  }, [getValues, key, name, reset]);

  const loadOptions: MultiSearchSelectProps<Option>['loadOptions'] = useCallback(
    async (filter?: { nextToken?: string, searchTerm?: string }) => {
      ...
      return { data: getData?.data?.nodes.map(i => ({ id: String(i.id), label: i[name as keyof AccountFilterNoRelations] })) as Option[], nextToken: getData?.data?.pageInfo?.endCursor }
  }, [key, name])
  ...
  const generateRenderProps = useCallback(() => {
    ...
    const mapping: RenderTypeMap = {
      ...
      [RenderTypes.PropertySelector]: {
        loadOptions: loadOptions,
        value: value[key] as string,
        renderSelectedItem: (item: any) => item && <SelectorBox>{item}</SelectorBox>,
        onChange: handleValueChange,
        renderOption: renderDropdownOption,
      },
    };
