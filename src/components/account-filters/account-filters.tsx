...
import { Option, renderDropdownOption } from '../ui-components';
import { Container, Form, InputSelect } from './account-filters.styles';
import { AccountFilterNoRelations, availableFields, fieldMappings } from './utils';
...
export const AccountFilters = ({ initialValue, handleChanges }: AccountFiltersProps) => {
  ...
  const onSubmit: SubmitHandler<AccountFilterNoRelations> = useCallback(data => handleChanges(data), [handleChanges]);
  const values = watch();
  ...
  const handleAddAnotherOption = (values: { id: string, label: string }[]) => {
    ...
    const newItems = values.filter(item => !currentValues[item.id as keyof AccountFilterNoRelations]).reduce((acc, next) => ({
      ...acc,
      [next.id]: {},
    }), {});
    const removedItems = Object.keys(currentValues).filter(key => !values.map(item => item.id).includes(key));
  ...
  const renderItem = useCallback((item: Option) => {
    ...
    return <Component key={item.id} value={isEmpty(baseValue) ? generateValue(Component.toString()) : baseValue} name={item.id } /> as JSX.Element;
  }, [values]);
    ...
    <FormProvider {...form}>
      <Container>
        <Form onSubmit={handleSubmit(onSubmit)}>
          <InputSelect
            isFullWidthInput
            options={availableFields.filter(i => !Object.keys(values).includes(i.id)) as any}
