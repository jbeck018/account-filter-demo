...
export type SelectBaseProps<T> = MenuBaseProps & {
  ...
  renderOption?: (item: T, options?: Partial<RenderOptionParams<T>>) => JSX.Element;
  renderPrevSelectedItem?: (item: T) => JSX.Element;
  renderSelectedItem?: (item: T) => JSX.Element;
  renderValueContainer?: (item: T) => JSX.Element;
  valueContainerWrapper?: (valueContainer: JSX.Element, options: { isOpen: boolean }) => JSX.Element;
};
...
export type WithPartial<T, K extends PropertyKey = PropertyKey> = Partial<Pick<T, Extract<keyof T, K>>> &
