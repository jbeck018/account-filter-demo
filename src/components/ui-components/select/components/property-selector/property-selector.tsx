...
import { AsyncSelect, SelectActions, SelectBaseProps, StaticSelect } from '../../helpers';
import { useLoadOptions, useOnHighlightedIndexChange, useSelectState } from '../../hooks';
import { SelectMenu as Menu } from '../menu/menu';

export type ControllerTransformer<TInput extends string = string, TOutput = any> = {
  transform?: {
...
> = SelectBaseProps<T> &
  (IsAsync extends true ? AsyncSelect<T> & { isSearchable?: boolean } : StaticSelect<T> & { isSearchable?: never }) & {
    customZIndex?: number;
    ...
    renderMenuFooter?: (payload: { inputValue: string; selectedItem?: T } & SelectActions<T>) => JSX.Element;
    renderMenuHeader?: (payload: { inputValue: string; selectedItem?: T } & SelectActions<T>) => JSX.Element;
...
const PropertySelectorInner = <T extends Record<'id', string>, IsAsync extends boolean = true>(
...
export const PropertySelector = memo(forwardRef(PropertySelectorInner)) as typeof PropertySelectorInner;
...
function SimplePropertySelector<T extends Record<'id', string>>({
  ...
  const menuHeader = React.useMemo(() => <>{renderMenuHeader?.(selectActions)}</>, [renderMenuHeader, selectActions]);
