...
import { SelectorBox, SelectorBoxProps } from '../../../selector-box';
import { AsyncSelect, Option, RenderOptionParams, SelectActions } from '../../helpers';
import {
  useIsUnmounted,
  useLoadOptions,
...
export type SearchSelectorProps<T extends Option> = AsyncSelect<T> & {
  ...
  renderSelectedItem?: (item: T, actions?: SearchSelectorActions<T>) => JSX.Element;
  showLabel?: boolean;
  suffix?: ReactNode;
  value?: T | null;
};

export const SearchSelector = memo(SearchSelectorInner) as typeof SearchSelectorInner;
...
function SearchSelectorInner<T extends Option>({
  ...
  const selectActions = React.useMemo<SearchSelectorActions<T>>(
    ...
    [closeMenu, inputValue, onBack, parentItem?.data, parentItem?.options?.searchTerm, selectItem, setInputValue]
  );
  ...
  const selectedItemEl = React.useMemo(() => {
    ...
    try {
      const selectedItemElement = React.cloneElement<SelectorBoxProps>(menuItem as any, {
  ...
  }, [selectedItem, renderSelectedItem, selectActions, renderOption, disabled, selectItem]);
  ...
  const valueContainer = selectedItem ? (
    ...
    React.cloneElement(placeholderElement, { onClick: disabled ? undefined : isOpen ? undefined : openMenu })
  ) : (
    <SelectorBox isEmpty={!selectedItem} onClick={disabled ? undefined : openMenu}>
      {placeholder}
    </SelectorBox>
  );
      ...
      <div>
        <div {...getComboboxProps({ disabled })}>
          <div ref={menuReferenceElement}>
            {valueContainerWrapper ? valueContainerWrapper?.(valueContainer as any, { isOpen }) : valueContainer}
          </div>
        </div>
