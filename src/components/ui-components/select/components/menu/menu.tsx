...
import { Arrow, arrowSize, LI, MenuContent, MenuDiv, UL } from './menu.styles';
import { modifiers } from './utils';

const menuItemHeight = () => 35;

export const SelectMenu = memo(MenuInner) as <T>(props: MenuProps<T>) => JSX.Element;
...
function MenuInner<T extends Element>({
  ...
  const { styles: popperStyles, attributes } = usePopper(menuReferenceElement?.current, menuElement, {
  ...
  useImperativeHandle(scrollToIndex, () => rowVirtualizer.scrollToIndex, [rowVirtualizer.scrollToIndex]);

  const menuArrow = <Arrow data-popper-arrow style={popperStyles.arrow} {...attributes.arrow} />;
  ...
  const renderMenuItemWrapper = useCallback(
    (virtualItem: VirtualItem<T>) => {
      ...
      return (
        <LI
          key={virtualItem.key}
          className={highlightedIndex === virtualItem.index && liHighligtedClassName}
          {...getItemProps && getItemProps({
            ...
            style: {
              ...
              ...(selectedItem as unknown as Record<'id', string>)?.id === (item as unknown as Record<'id', string>).id &&
              { background: '#E6FDF9'},
              ...highlightedIndex === virtualItem.index && { background: '#EDF2F3'},
            },
          })}
        >
          {isPrevSelected && renderPrevSelectedItem ? renderPrevSelectedItem(item) : renderMenuItem(item)}
        </LI>
      );
    },
    [items, checkIsAlreadySelected, selectedItem, highlightedIndex, liHighligtedClassName, getItemProps, renderPrevSelectedItem, renderMenuItem]
  );
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      <div {...getMenuProps && getMenuProps({ ref: ulRef as any, style: { outlineColor: 'transparent' } })}>
