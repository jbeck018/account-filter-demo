import React, { ReactNode, createContext, memo, useCallback, useContext, useMemo, useRef } from 'react';
...
export type MultiSearchSelectProps<O extends Option> = MultiSearchSelectOverrideProps<O> &
  Omit<SelectBaseProps<O>, keyof MultiSearchSelectOverrideProps<O>> &
  Omit<AsyncSelect<O>, keyof MultiSearchSelectOverrideProps<O>> & {
    preOptions?: never;
  };

export const onChangeOnSelectContext = createContext({ triggerOnChangeOnSelect: false });

/**
 * @description Use this, if you want to trigger onChange after selecting an option in multiSelector
 */
export const OnChangeOnSelectContextProvider = memo<{ children: ReactNode; trigger?: boolean }>(
  ({ children, trigger }) => {
    const value = useMemo(() => ({ triggerOnChangeOnSelect: trigger || false }), [trigger]);

    return <onChangeOnSelectContext.Provider value={value}>{children}</onChangeOnSelectContext.Provider>;
  }
);

export const MultiSearchSelector = memo(MultiSearchInner) as typeof MultiSearchInner;
...
function MultiSearchInner<T extends Option>({
  ...
  const { onHighlightedIndexChange, scrollToIndexFunctionRef } = useOnHighlightedIndexChange();

  const { onSelect: onSelectParent, resetRecursiveState, currentItem: parentItem, onBack } = useRecursiveSelect<T>();
  const { resetState, setState, state } = useSelectState<T>({ parent: parentItem?.data });
  ...
  const { getSelectedItemProps, getDropdownProps, addSelectedItem, removeSelectedItem } = useMultipleSelection<T>({
    ...
    onSelectedItemsChange: ({ selectedItems = [] }) => {
      const uniqSelectedItems = hideSelectedItems ? selectedItems : uniqBy(selectedItems, 'id');
  ...
  const data = React.useMemo(
    () => (hideSelectedItems ? state.data.filter(item => !find(selectedItems, { id: item.id })) : state.data),
    ...
    stateReducer: (_state, actionAndChanges) => {
      ...
      if (
        ...
        const newChanges = {
          ...
          isOpen: !hideMenuOnSelect && (isRemovedItemWhileMenuOpen.current || !!changes.selectedItem), // keep the menu open after selection.
        };
  ...
  const selectActions = React.useMemo<Actions<T>>(
    ...
    [addSelectedItem, closeMenu, inputValue, onBack, parentItem, removeSelectedItem, setInputValue]
  );
  ...
  const renderSelectedItemWrapper = useCallback(
    (selectedItem: T, index: number) => {
      const renderItem = () => {
        ...
        if (renderSelectedItem) return renderSelectedItem(selectedItem as T, selectActions);

        const menuItem = renderOption(selectedItem as T, { type: 'value' });

        try {
          if (shouldRenderOptionAsPill) {
            const selectedItemElem = React.cloneElement<PillProps>(menuItem as any, {
          ...
          const selectedItemElem = React.cloneElement<SelectorBoxProps>(menuItem as any, {
            isRemovable: !disabled,
            isRemovableOnHover: !disabled,
            variant: 'hoverable',
            onRemove: () => {
      ...
      return (
        <span key={`selected-item-${index}`} style={{ ...style, ...(isFullWidthInput && { width: '100%', })}} {...props}>
          {renderItem()}
        </span>
      );
    },
