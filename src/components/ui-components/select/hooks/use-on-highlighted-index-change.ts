...
export const useOnHighlightedIndexChange = () => {
  ...
  const onHighlightedIndexChange = useCallback(({ highlightedIndex = 0 }: { highlightedIndex?: number }) => {
    if (highlightedIndex > 0) scrollToIndexFunctionRef.current?.(highlightedIndex);
  }, []);
